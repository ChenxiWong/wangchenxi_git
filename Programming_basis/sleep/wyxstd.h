...
static int timeout_second_num = 100;
    ...
    public:
        ...
        bool conservative_safety_call(Cnode* p_Cnode, uint64_t* p_m_counter, EQA enum_queue_action)
        {
            ...
            switch(enum_queue_action)
            {
                ...
                default:
                    {
                        {DBGCODE(cerr<<"Unknown action appeared, Please check your code!"<<endl;
                                exit(EXIT_FAILURE);
                                )}
                    }
            }
...
class wyxCThreadPool
{
    private:
        ...
        // create all threads of threadpool
        int m_create()
        {
...
static void* m_thread_work(void* thread_data)
{
    ...
    while(1)
    {
        ...
        {
            {DBGCODE(clog<<"threadpool has no task..."<<endl;)}
            sleep(1);
            --timeout_second_num;
            if(timeout_second_num == 0)
            {
                cout<<" May Be Success !"<<endl;
                exit(EXIT_FAILURE);
            }
        }
